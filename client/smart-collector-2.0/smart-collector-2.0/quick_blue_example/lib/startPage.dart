...
class _startPageState extends State<startPage> {
  ...
  Future<void> loadConfigVariables() async {
    ...
    widget.globals.receivedENVJsonValues=[];
    widget.globals.receivedORIJsonValues=[];//TO DO save saved values not posted.
  ...
  Future<void> saveConfigVariables() async {
    ...
    await widget.globals.prefs.setString(
        'receivedENVJsonValues', jsonEncode(widget.globals.receivedENVJsonValues));
    await widget.globals.prefs.setString(
        'receivedORIJsonValues', jsonEncode(widget.globals.receivedORIJsonValues));
  }

  //When a characteristic change he reads the value and decodes it and saves into different variables
  void _handleValueChange(
    ...
    else if (characteristicId == "8e7c2dae-0003-4b0d-b516-f525649c49ca") {//ENV
      Map<String, dynamic> jsonObj = parseENVData(value, 8);  
      setState(() {// Add the parsed data to the receivedValues list of the specific characteristic
        widget.globals.receivedENVJsonValues.add(jsonObj);
      });
    }
    else if (characteristicId == "8e7c2dae-0004-4b0d-b516-f525649c49ca") {//ENV
  ...
  Map<String, dynamic> parseENVData(Uint8List value, int ArrayLength) {
    ...
    List<double> floatValuesEnv = List<double>.filled(8, 0.0);
    ...
    for (int i = 0; i < envData.length; i++) {
      //conversion ENV from int to float
      if (i>0 && i<4) {
        floatValuesEnv[i] = envData[i] / 100;
      }
    }
    Map<String, dynamic> jsonObj = {
        "timestamp": DateTime.now().millisecondsSinceEpoch,
        "values": floatValuesEnv,
      };
      //To visualize jsonObj
      //String jsonDataString = jsonEncode(jsonObj);
      //print(jsonDataString);
  ...
  Map<String, dynamic> parseORIData(Uint8List value, int ArrayLength) {
    final byteData = ByteData.view(value.buffer);
    final oriData = List<int>.filled(ArrayLength, 0);
    List<double> floatValues = List<double>.filled(3, 0.0);
    for (var i = 0; i < oriData.length; i++) {
      if (i < value.lengthInBytes ~/ 2) {
        double doubleValue = byteData.getFloat32(i * 4, Endian.little);
        floatValues[i] = doubleValue;
  ...
  Future<void> sendData() async {
    if(widget.globals.option1){
    ...
    else if (widget.globals.option2) {
      print("Sending data to " + widget.globals.measureName.toString());
      String jsonDataString = jsonEncode(widget.globals.receivedENVJsonValues);
      print(jsonDataString);
      var headers = {
        'Content-Type': 'application/json',
        'Authorization': widget.globals.deviceToken
      ...
      if (response.statusCode == 200) {
        print(await response.stream.bytesToString());
        widget.globals.receivedENVJsonValues=[];//reset
        ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text("Values sended correctly!")),
        );
      } else {
    ...
    else if (widget.globals.option3) {
      print("Sending data to " + widget.globals.measureName.toString());
      String jsonDataString = jsonEncode(widget.globals.receivedORIJsonValues);
      print(jsonDataString);
      var headers = {
        'Content-Type': 'application/json',
        'Authorization': widget.globals.deviceToken
      ...
      if (response.statusCode == 200) {
        print(await response.stream.bytesToString());
        widget.globals.receivedORIJsonValues=[];//reset
        ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text("Values sended correctly!")),
        );
      } else {
